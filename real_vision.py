...
class ThreadedCamera:
    """
    Dedicated high-speed thread to grab network stream frames.
    Eliminates OpenCV internal buffer bloat so live phone stream never accumulates lag.
    """

    def __init__(self, src: str):
        print(f"[STREAM] Connecting to wireless stream: {src}")
        ...
        if not self.cap.isOpened():
            print(f"[ERROR] Could not open wireless stream at: {src}")
            self.running = False
            return

        # Start daemon background reader
        self.thread = threading.Thread(target=self._reader, daemon=True)
        self.thread.start()
        print("[STREAM] Low-latency wireless capture thread active.")
...
class UGVVisionEngine:
    ...
    def __init__(self, onnx_model_path: str = "segformer_b0.onnx"):
        ...
        # Prioritize DirectML (DirectX 12 GPU on RTX 4050) -> CPU fallback
        providers = []
    ...
    def infer(self, frame_bgr: np.ndarray) -> Tuple[np.ndarray, np.ndarray, float]:
        ...
        # 1. Preprocess (Resize 512x512 -> RGB -> Normalize)
...
def run_pipeline(source=0, alpha: float = 0.45):
    """Launches the real-time traversability perception feed."""
    engine = UGVVisionEngine("segformer_b0.onnx")

    # Detect if source is wireless IP stream
    ...
    if is_network_stream:
        # Standardize IP Webcam URL if user only passes IP:Port
        ...
        # Give stream a moment to buffer first frame
        time.sleep(1.0)
    else:
    ...
    if not cap.isOpened():
        ...
        print("Tips for IP Webcam:")
        print(" 1. Check if both phone and laptop are reachable.")
        print(" 2. Make sure you included the port (e.g. 192.168.1.5:8080).")
        print(" 3. If campus/library Wi-Fi blocks peer-to-peer, use your phone's personal Hotspot.\n")
        return
    ...
    print(" 🟩 GREEN  = Traversable Ground / Walkable Floor")
    print(" 🟥 RED    = Obstacles (Desks, Chairs, Walls, Persons)")
    ...
    while cap.isOpened():
        ...
        # Alpha-blend color overlay over camera frame
        ...
        # -------------------------------------------------------------
        # Authentic UGV Telemetry Bar (Top Left)
        # -------------------------------------------------------------
        cv2.rectangle(blended, (12, 12), (380, 78), (15, 15, 15), -1)
        cv2.rectangle(blended, (12, 12), (380, 78), (50, 50, 50), 1)

        cv2.putText(blended, "UGV REAL-TIME TRAVERSABILITY", (22, 32),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.48, (255, 255, 255), 1, cv2.LINE_AA)

        gpu_status_color = (0, 255, 180) if "RTX" in engine.device_name else (0, 200, 255)
        cv2.putText(blended, f"DEVICE: {engine.device_name}", (22, 50),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.40, gpu_status_color, 1, cv2.LINE_AA)

        src_label = "PHONE WIRELESS" if is_network_stream else "LOCAL CAMERA"
        cv2.putText(blended, f"GPU INFER: {gpu_latency:.1f}ms | {fps_smooth:.1f} FPS [{src_label}]", (22, 68),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.38, (220, 220, 220), 1, cv2.LINE_AA)

        # -------------------------------------------------------------
        # Nav2 Metric Costmap (Picture-in-Picture, Top Right)
        # -------------------------------------------------------------
        ...
        # -------------------------------------------------------------
        leg_y = h - 20
        cv2.rectangle(blended, (12, leg_y - 20), (430, leg_y + 10), (15, 15, 15), -1)
        # Green patch
        cv2.rectangle(blended, (20, leg_y - 12), (32, leg_y), (34, 180, 34), -1)
        cv2.putText(blended, "Traversable Ground", (38, leg_y - 2), cv2.FONT_HERSHEY_SIMPLEX, 0.38, (255, 255, 255), 1)
        # Red patch
        cv2.rectangle(blended, (180, leg_y - 12), (192, leg_y), (40, 40, 220), -1)
        cv2.putText(blended, "Obstacle", (198, leg_y - 2), cv2.FONT_HERSHEY_SIMPLEX, 0.38, (255, 255, 255), 1)
        # Gray patch
        cv2.rectangle(blended, (275, leg_y - 12), (287, leg_y), (70, 70, 70), -1)
        cv2.putText(blended, "Background", (293, leg_y - 2), cv2.FONT_HERSHEY_SIMPLEX, 0.38, (255, 255, 255), 1)

        # Render frame
        cv2.imshow("Autonomous UGV Traversability [RTX 4050]", blended)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--url", type=str, default=None, help="Wireless Phone IP Webcam URL (e.g. 192.168.1.10:8080)")
    parser.add_argument("--webcam", type=int, default=0, help="Local Webcam device ID (default: 0)")
    parser.add_argument("--video", type=str, default=None, help="Path to video file")
    parser.add_argument("--alpha", type=float, default=0.45, help="Mask overlay alpha (default: 0.45)")
